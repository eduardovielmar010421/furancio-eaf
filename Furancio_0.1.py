...
# ==========================================
st.set_page_config(
    page_title="Furancio EAF - Dashboard",
    layout="wide",
...
def load_data():
    try:
        ...
        df_reporte = pd.read_excel(
            "reporteporcolada.xlsx", header=17, engine="openpyxl"
        )
...
tecnicos_seleccionados = st.sidebar.multiselect(
    "Filtrar por Técnico:", options=lista_tecnicos, default=lista_tecnicos
)

lista_grados = df_toda[COL_GRADO].dropna().unique().tolist()
grados_seleccionados = st.sidebar.multiselect(
    "Filtrar por Grado:", options=lista_grados, default=lista_grados
)
...
# ==========================================
col_colada_rep = (
    "No.Colada" if "No.Colada" in df_reporte.columns else "No. Colada"
)
col_grado_rep = (
    "Grado Acero" if "Grado Acero" in df_reporte.columns else COL_GRADO
)
...
if (
    len(df_reporte) > 0
    and col_colada_rep in df_reporte.columns
    and col_grado_rep in df_reporte.columns
):
...
if (
    ultima_colada == "N/D" or grado_ultima in ["N/D", "nan", "NaN"]
) and len(df_filtrado) > 0:
...
prom_vac_vac = (
    df_filtrado["Min VAC/VAC"].mean() if "Min VAC/VAC" in df_filtrado.columns else 0
)
prom_kwh_tcm = (
    df_filtrado["KWh TCM[+]"].mean() if "KWh TCM[+]" in df_filtrado.columns else 0
)
prom_kwh_tpqb = (
    df_filtrado["KWh TPQB"].mean() if "KWh TPQB" in df_filtrado.columns else 0
)
prom_tpqb_col = (
    df_filtrado["TPQB/Col"].mean() if "TPQB/Col" in df_filtrado.columns else 0
)
...
with col1:
    ...
    st.markdown(
        '<div class="section-header">Tiempos de Ciclo</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="section-header">Consumos Eléctricos</div>',
        unsafe_allow_html=True,
    )
...
with col2:
    ...
    tab_proceso, tab_produccion, tab_escoria = st.tabs(
        ["🔥 Proceso EAF", "📊 Producción", "🧪 Análisis Escoria"]
    )

    with tab_proceso:
        if (
            "No. Colada" in df_filtrado.columns
            and "KWh TPQB" in df_filtrado.columns
            and "KWh TCM[+]" in df_filtrado.columns
        ):
            ...
            fig_linea.add_hline(
                y=340, line_dash="dash", line_color="#8b949e", layer="below"
            )
            fig_linea.add_hline(
                y=360, line_dash="dash", line_color="#f85149", layer="below"
            )
            ...
            st.info(
                "💡 Las columnas requeridas para el gráfico de líneas no están disponibles."
            )
    ...
    with tab_escoria:
        st.dataframe(
            df_escorias.head(10), use_container_width=True, hide_index=True
        )
